...
const CouponCard = ({d, shop_Link}) => {
    ...
    return (
        <div className="bg-white p-6 text-center flex flex-col *:mb-2 justify-between rounded-2xl">
         ...
         <h3 className="text-3xl text-center font-bold text-[#f57f25]">{description}</h3>
          ...
          {/* <p>{moment(expiry_date, "DDMMYYYY").fromNow()}</p> */}
           ...
           <CopyToClipboard text={coupon_code}>
              <button onClick={coped}  className="btn bg-[#f57f25] text-white font-semibold cursor-pointer">
                {coupon_code}
              </button>
            </CopyToClipboard>
